...
public class HW7TestDriver {
    ...
    private void listChildren(String graphName, String parentName) {
    	...
    	for(Edge node : children) {
    		outputChildren += " " + node.getNode() + "(" + String.format("%.3f", node.getLabel()) + ")";
    	}
    ...
    private void findPath(String graphName, String source, String destination) {
    	...
    	if(allNodes.contains(source) && allNodes.contains(destination)) {
    		...
    		List<Edge<String, Double>> path = MarvelPaths2.findMinimumCostPath(graph, source, destination);
    		if(path != null) {
    			path.remove(0);
        		double total = 0.0;
    			for(Edge<String, Double> edge : path) {
    				...
    				double number = edge.getLabel();
    				result += tempNode + " with weight " + String.format("%.3f", (number - total));
    				total = number;
    				
    			}
    			result += "\n" + "total cost: " + String.format("%.3f", total);
    		}else {
    			result += "\n" + "no path found";
    		}
